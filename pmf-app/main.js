			...
			if(results.length){
				for(var i=0;i<results.length;i++){
					...
					if(speedVal<=0){
						...
						fixDatav1(dateStart,dateEnd,vehicleNum,company);
						break;
					}
				}
			}
		...
		if(results.length){
			for(var i=results.length-1;i>0;i--){
				...
				if(speedVal<=0){
					...
					removeLargeGaps(dateStart,dateEnd,vehicleNum,company);
					break;
				}
			}
			return;
